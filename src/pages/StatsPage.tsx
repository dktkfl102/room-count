...
const moveBucketMs = (bucketStartMs: number, unit: StatsUnit, amount: number) => {
  if (unit === "day") {
...
function StatsPage() {
  ...
  const points = useMemo(() => {
    ...
    const starts = Array.from({ length: size }, (_, index) => moveBucketMs(currentStart, unit, index - (size - 1)))
  ...
  return (
    <section className="mx-auto w-full max-w-4xl">
      ...
      <p className="mt-1 text-sm text-muted-foreground sm:text-base">
        {UNIT_CONFIG[unit].rangeText} 기준으로 핵심 지표를 확인할 수 있습니다.
      </p>
        ...
        {(["day", "week", "month"] as StatsUnit[]).map((nextUnit) => (
          <button
            ...
            className={[
              ...
              unit === nextUnit ? "bg-primary text-primary-foreground" : "hover:bg-muted",
            ].join(" ")}
          >
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
        <div className="rounded-lg border bg-muted/30 p-4">
          <p className="text-sm text-muted-foreground">선택 기간 매출</p>
          <p className="mt-2 text-xl font-bold sm:text-2xl">{formatCurrency(totals.totalSales)}원</p>
        </div>
        <div className="rounded-lg border bg-muted/30 p-4">
      ...
      <div className="mt-5 rounded-lg border p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <h3 className="text-base font-semibold sm:text-lg">추이 그래프</h3>
            ...
            {([
              ...
              <button
                ...
                className={[
                  ...
                  metric === nextMetric ? "bg-primary text-primary-foreground" : "bg-background",
                ].join(" ")}
              >
                {label}
              </button>
        ...
        >
          {points.map((point, index) => {
            ...
            const heightPercent = value > 0 ? Math.max(8, (value / maxValue) * 100) : 0
            return (
              <div key={point.key} className="flex h-full flex-col items-center justify-end gap-1">
                ...
                <div className="text-[10px] text-muted-foreground sm:text-xs">{point.label}</div>
              </div>
            )
          })}
        </div>
