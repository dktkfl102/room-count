...
function PriceSettingsPage() {
    ...
    const handleResetToDefault = () => {
        ...
        setSaved(
            "기본값(시간/음료/소주/맥주)으로 되돌렸습니다. 저장 버튼을 눌러 반영해 주세요.",
        );
    };
        ...
        <section className="mx-auto w-full max-w-2xl">
            ...
            <div className="mt-5 space-y-3">
                {formItems.map((item, index) => (
                    <div
                        key={item.id}
                        className="grid grid-cols-1 gap-2 rounded-lg border p-3 sm:grid-cols-[1fr_160px_120px_auto]"
                    >
                        ...
                        <label className="flex h-11 items-center gap-2 rounded-lg border px-3">
                            <input
                                type="checkbox"
                                checked={item.includeInActualSales !== false}
                                disabled={item.category === "time"}
                                onChange={(event) =>
                                    handleChangeItem(
                                        index,
                                        "includeInActualSales",
